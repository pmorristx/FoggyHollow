...
class MineTrack(jmri.jmrit.automat.AbstractAutomaton) :
	...
	def postDepartureTime(self, delaySeconds):
		import time

		now = time.time()
		timeStr = time.strftime("%I:%M", time.localtime(now + delaySeconds))		
		self.departureBoard.setField("Departs", timeStr, self.trackNumber-1, 0)	
	...
	def reset(self):
		...
		try :		
			if (sensors.provideSensor("BOT").getState() != ACTIVE) :					
				...
				self.locomotive.setSpeed(self.fastSpeed * self.speedScale)
...
#			print "BOTAction"
	...
	def tunnelNorth(self):	
		...
		if ((self.currentState == -3) and (not self.reseting)):
			...
			self.waitMsec(2000)
			self.locomotive.setSpeed(self.mediumSpeed * self.speedScale)			
		return 0		
	...
	def bridge2South(self):	
		...
		if (self.currentState == 3 and (not self.reseting) and (self.automationSwitch.getState() == ACTIVE)) :
			...
			elif (java.util.Random().nextInt(100) < 70) :
				self.locomotive.longWhistle(1)
			else :
				self.locomotive.tootWhistle(1)
	...
	def serviceLocomotive(self):
		...
		secsToDepart = self.postDepartureTime(delaySeconds)
		...
		if (self.needBoilerPurge) :
			...
			self.waitMsec((60 + java.util.Random().nextInt(5)) * 1000)
		else :
			self.needBoilerPurge = True
			
		self.dumpAshes(20)
		self.waitMsec((45 + java.util.Random().nextInt(5)) * 1000)
	...
	def returnToMine(self):	
		...
		self.locomotive.setSpeed(self.creepSpeed * self.speedScale) # Start going back	
		self.waitMsec(3000)			
		self.locomotive.setSpeed(self.extraSlowSpeed * self.speedScale) # Start going back
