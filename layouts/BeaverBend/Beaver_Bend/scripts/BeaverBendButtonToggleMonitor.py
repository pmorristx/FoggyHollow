...
#
class BeaverBendButtonToggleMonitor(jmri.jmrit.automat.AbstractAutomaton) :

    def init(self):
        #
        # Buttons being initialized and tracked. Only track water tower
        ButtonList = [22,24]
    ...
    class TrackDevice(java.beans.PropertyChangeListener) :
        ...
        def init(self, btnNumber, panelEditor, parent) :
            ...
            self.Indicator = sensors.getSensor("IS" + str(btnNumber) + ":K")
            if (btnNumber == 24) :
                self.Turnout = turnouts.provideTurnout("NT" + str(btnNumber))
            else :
                self.Turnout = turnouts.provideTurnout("NT1")            
            ...
            self.chainPulley = jmri.jmrit.Sound(jmri.util.FileUtil.getExternalFilename("preference:resources/sounds/ChainPulley.wav"))            
...
BeaverBendButtonToggleMonitor().start()          # create one of these, and start it running
