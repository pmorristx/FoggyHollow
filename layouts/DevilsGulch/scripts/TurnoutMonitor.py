...
###            self.Device.setState(self.DeviceValue)
            ...
            if (self.DeviceValue == "N") :
                ...
                sensors.getSensor( signalId + ":NWK").setState(ACTIVE)
                sensors.getSensor( signalId + ":WL").setState(ACTIVE)
                sensors.getSensor( signalId + ":RWK").setState(INACTIVE)

#                sensors.getSensor( "CTC:S" + self.DeviceNumber + "NS").setState(ACTIVE)
#                sensors.getSensor( "CTC:S" + self.DeviceNumber + "RS").setState(INACTIVE)
#                if (self.DeviceNumber == 1 or self.DeviceNumber == 9) :
#                    sensors.getSensor( "CTC:S" + self.DeviceNumber + "FB").setState(ACTIVE)

                #signals.getSignalHead("VH:T" + self.DeviceNumber + "N").setAppearance(GREEN)
                #signals.getSignalHead("VH:T" + self.DeviceNumber + "R").setAppearance(RED)

            else :
                turnouts.provideTurnout("NT" + self.DeviceNumber).setState(THROWN)
                self.parent.waitMsec(100)
                sensors.getSensor( signalId + ":NWK").setState(INACTIVE)
                sensors.getSensor( signalId + ":RWK").setState(ACTIVE)
                sensors.getSensor( signalId + ":WL").setState(INACTIVE)

#                sensors.getSensor( "CTC:S" + self.DeviceNumber + "NS").setState(INACTIVE)
#                sensors.getSensor( "CTC:S" + self.DeviceNumber + "RS").setState(ACTIVE)
#                if (self.DeviceNumber == 1 or self.DeviceNumber == 9) :
#                    sensors.getSensor( "CTC:S" + self.DeviceNumber + "FB").setState(INACTIVE)

##                signals.getSignalHead("VH:T" + self.DeviceNumber + "N").setAppearance(RED)
#               signals.getSignalHead("VH:T" + self.DeviceNumber + "R").setAppearance(GREEN)
        ...
        def sendTimeoutHandler(self, event) :
            ...
            #  Move the actual throwing of the turnout to the end to prevent the CTC lights from turning on too soon.  This could be reactivated
            #  if/when we use sensor lights instead of hardwired in the tortoise motor wire.
...
#                turnouts.provideTurnout("NT" + self.DeviceNumber).setState(THROWN)
        ...
        def pauseTimeoutHandler(self, event) :
            ...
            sensors.getSensor( newName + ":NWK").setState(INACTIVE)
            sensors.getSensor( newName + ":RWK").setState(INACTIVE)
...
#            if (self.Device.getValue() == "N") :
#                sensors.getSensor( "IS" + self.DeviceNumber + ":NK").setState(ACTIVE)
#                sensors.getSensor( "IS" + self.DeviceNumber + ":RK").setState(INACTIVE)
#                signals.getSignalHead("VH:T" + str(self.DeviceNumber) + "N").setAppearance(GREEN)
#                signals.getSignalHead("VH:T" + str(self.DeviceNumber) + "R").setAppearance(RED)
#            elif (self.Device.getValue() == "R") :
#                sensors.getSensor( "IS" + self.DeviceNumber + ":NK").setState(INACTIVE)
#                sensors.getSensor( "IS" + self.DeviceNumber + ":RK").setState(ACTIVE)
#                signals.getSignalHead("VH:T" + str(self.DeviceNumber) + "N").setAppearance(RED)
#                signals.getSignalHead("VH:T" + str(self.DeviceNumber) + "R").setAppearance(GREEN)
